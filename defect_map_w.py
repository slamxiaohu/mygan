...
def create_defect_map(image, yolo_annotations):
    defect_map = image.copy()
    img_height, img_width = image.shape[:2]

    for annotation in yolo_annotations:
        x1, y1, x2, y2 = yolo_to_bbox(annotation, img_width, img_height)
        cv2.rectangle(defect_map, (x1, y1), (x2, y2), (255, 255, 255), -1)
    ...
    for filename in os.listdir(image_dir):
        ...
        if os.path.exists(annotation_path):
            try:
                ...
                defect_map = create_defect_map(image, annotations)
...
output_dir = os.path.join(base_dir, 'defect_map_w')
